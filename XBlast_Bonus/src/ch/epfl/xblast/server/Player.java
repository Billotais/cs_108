...
public final class Player
{
    ...
    private final Sq<Boolean> canSeeSequence;
    
    /**
     * Create a new Player with an ID, a sequence of lifestates, a sequence of directed positions, 
     *              the maximum number of bombs, the range of the bombs, and sequence of his ability to see the board.
     * 
     * @param id
     *              a player ID.
     * 
     ...
     * @param canSeeSequence
     *              a sequence giving for each game tick if the player can see the board or not
     * 
    ...
    public Player(PlayerID id, Sq<LifeState> lifeStates,
            Sq<DirectedPosition> directedPos, int maxBombs, int bombRange, Sq<Boolean> canSeeSequence)
    {
        ...
        this.canSeeSequence = canSeeSequence;
        
    }
    
    /**
     * Create a new Player with an ID, a sequence of lifestates, a sequence of directed positions, 
     *              the maximum number of bombs, and the range of the bombs, and that can always see the board.
     * 
     * @param id
     *              a player ID.
     * 
    ...
    {
        this(id, lifeStates, directedPos, maxBombs, bombRange, Sq.constant(true));
    }
    ...
    /**
     * Says if the player can see the board
     * @return true
     *              if he can
     *         false
     *              if he can't   
    ...
    public boolean canSee()
    {
        ...
        return canSeeSequence.head();
    }
    
    /**
     * Gives the sequence representing the ability of the player to see the board
     * @return the sequence representing the ability of the player to see the board
     */
    public Sq<Boolean> canSeeSq()
    {
        return canSeeSequence;
    }
    ...
    {
        return new Player(id, lifeStates, directedPos, newMaxBombs, bombRange,canSeeSequence.tail());
    }
    ...
    {
        return new Player(id, lifeStates, directedPos, maxBombs, newBombRange,canSeeSequence.tail());
    }
    
    /**
     * Return a Player with a changed amout of lifes and thaht is invicible for a shrot duration
     * @param newLifes 
     *          the new number of lifes
     * @return the new Player 
     */
    
    public Player withLife(int newLifes)
    {
        return new Player(id, Sq.constant(new LifeState(newLifes, State.INVULNERABLE)).limit(Ticks.PLAYER_INVULNERABLE_TICKS).concat(Sq.constant(new LifeState(newLifes, State.VULNERABLE))), directedPos, maxBombs, bombRange,canSeeSequence.tail());
    }
    /**
     * Return a player with a changed position. The new position if the same as the giving player position
     * @param p 
     *          the given Player we want to take the position of
     * @return  the player with a changed posiion
     */
    public Player withPosOf(Player p)
    {
        return new Player(id, lifeStates, p.directedPos, maxBombs, bombRange,canSeeSequence.tail());
    }
    
    /**
     * Return a player that is now blind for a short duration
     * @return a blind player
     */
    public Player withBlindness()
    {
        Sq<Boolean> canSeeSq = Sq.repeat(15, false).concat(Sq.repeat(1,true));
        
        for (int i = 0; i < 8; ++i)
        {
             canSeeSq = canSeeSq.concat(Sq.repeat(15, false).concat(Sq.repeat(1,true)));
            
        }
        canSeeSq = canSeeSq.concat(Sq.constant(true));
        return new Player(id, lifeStates,directedPos,maxBombs,bombRange,canSeeSq);
    }
