...
public final class GameState
{
    ...
    public GameState next(Map<PlayerID, Optional<Direction>> speedChangeEvents, Set<PlayerID> bombDropEvents)
    {
        ...
        Board board1 = nextBoard(board, consumedBonuses, blastedCells(blasts1),Ticks.TOTAL_TICKS-ticks());
    ...
    private static Board nextBoard(Board board0, Set<Cell> consumedBonuses, Set<Cell> blastedCells1, double remaining)
    {
            ...
            {   
                ...
                {              
                    ...
                    {                 
                        ...
                        case 7:
                        case 8:
                            blockSpawned = Block.BONUS_TP;
                            break;
                        case 9:
                            blockSpawned = Block.BONUS_LIFE;
                            break;
                        case 10:
                        case 11:
                            blockSpawned = Block.BONUS_BLIND;
                            break;
                    }
        ...
        if (remaining % 10 == 0 && remaining <= time && Main.SUBIT_DEATH)
        {
            List<Cell> provList= new ArrayList<>(Cell.SPIRAL_ORDER);
            Collections.reverse(provList);
            listForTheNewBoard.set(provList.get((int)((time-remaining)/10)).rowMajorIndex(), Sq.constant(Block.INDESTRUCTIBLE_WALL));
        }
            ...
            Map<PlayerID, Optional<Direction>> speedChangeEvents)
    {
        ...
        List<PlayerID> playersHavingBlindness = new ArrayList<>();
        for (Player onePlayer : players0)
        {
            ...
            if( !board1.blockAt(onePlayer.position().containingCell()).canHostPlayer())
            {
                maybeNewSqOfLifeStates = Sq.constant(new LifeState(0, State.DEAD));
            }
            
            
            Player newlyCreatedPlayer = new Player(onePlayerID, maybeNewSqOfLifeStates, maybeChangedDirectedPositions, onePlayer.maxBombs(), onePlayer.bombRange(), onePlayer.canSeeSq().tail());
            if (playerBonuses.containsKey(onePlayerID) && !posOfConsummedBonuses.contains(onePlayer.position())) 
            {
                if (playerBonuses.get(onePlayerID) == Bonus.BLIND)
                {
                    
                    playersHavingBlindness.addAll(Arrays.asList(PlayerID.values()));
                    playersHavingBlindness.remove(onePlayerID);
                    

                }
                
                List<Player> provList = new ArrayList<>();
                for (Player provP : players0)
                {
                    if (provP.isAlive())
                    {
                        provList.add(provP);
                ...
                provList.remove(onePlayer);
                if (provList.size() > 0)
                {
                    int rdPlayer = RANDOM.nextInt(provList.size());
                    
                    players1.add(playerBonuses.get(onePlayerID).applyTo(newlyCreatedPlayer,provList.get(rdPlayer)));
                    posOfConsummedBonuses.add(onePlayer.position());
                }
                else
                {
                    players1.add(playerBonuses.get(onePlayerID).applyTo(newlyCreatedPlayer,newlyCreatedPlayer));
                    posOfConsummedBonuses.add(onePlayer.position());
                }    
            }
        ...
        List<Player> players1WIthBlindness = new ArrayList<>();
        
        
        for (Player pMayBeWithBlindness : players1)
        {
            if (playersHavingBlindness.contains(pMayBeWithBlindness.id()))
            {
                
                players1WIthBlindness.add(pMayBeWithBlindness.withBlindness());
            }
            else
            {
                players1WIthBlindness.add(pMayBeWithBlindness);
            }   
        }
        
        return players1WIthBlindness;
    }
