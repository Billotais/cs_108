...
public final class GameStateDeserializer
{
    ...
    private static final ImageCollection winnerCollection = new ImageCollection("winner");
    /**
    ...
    public static GameState deserializeGameState(List<Byte> listOfBytes)
    {
        ...
        List<Player> playersList = playerDeserializer(provListOfBytes.subList(0, 20));
        
        return new GameState(playersList, boardImages, explosionsImages, scoreDeserializer(playersList), timeDeserializer(provListOfBytes.get(20)),winnerDeserializer(provListOfBytes.get(21)));
    ...
    { 
        ...
        for (int i = 0; i < 4; ++i)
        {
            int lives = Byte.toUnsignedInt(listOfByte.get(i*5));
            SubCell pos = new SubCell(Byte.toUnsignedInt(listOfByte.get(i*5 + 1)), Byte.toUnsignedInt(listOfByte.get(i*5 + 2)));
            list.add(new Player(PlayerID.values()[i], lives, pos, playerCollection.imageOrNull(listOfByte.get(i*5 + 3)), (listOfByte.get(i*5 + 4) == 1))); 
        }
    ...
    private static Image winnerDeserializer(byte b)
    {
      
        return winnerCollection.imageOrNull(b);
    }
}
