...
public final class XBlastComponent extends JComponent
{
    ...
    public void paintComponent(Graphics g0)
    {
        ...
        boolean canSeeBoard = true;
        Graphics2D g = (Graphics2D) g0;
        for (Player clientP : gameState.getPlayers())
        {
            if (clientP.getId() == id)
            {
                if (! clientP.canSee())
        ...
        {
            g.setColor((rd.nextInt(2) == 1 ? Color.black : Color.white));
            
            //g.setColor(new Color(rd.nextInt(255), rd.nextInt(255), rd.nextInt(255)));
            g.fillRect(0, 0, Cell.COLUMNS*BLOCK_IMAGE_WIDTH, Cell.ROWS*BLOCK_IMAGE_HEIGHT);
            
        }
        
    
        g.drawImage(gameState.getWinnerImage(), 285, 210,null);
